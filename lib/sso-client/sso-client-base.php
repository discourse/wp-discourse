...
class SSOClientBase extends DiscourseBase {
	...
	protected function get_discourse_sso_link_markup( $link_options = array() ) {
		...
		if ( ! empty( $user_id ) ) {
			...
			$link_account_text = ! empty( $options['link-to-discourse-text'] ) ? $options['link-to-discourse-text'] : '';
			$anchor            = ! empty( $link_options['link'] ) ? $link_options['link'] : $link_account_text;
		} else {
			$login_text = ! empty( $options['external-login-text'] ) ? $options['external-login-text'] : '';
