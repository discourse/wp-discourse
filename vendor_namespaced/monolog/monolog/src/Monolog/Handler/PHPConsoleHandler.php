...
use WPDiscourse\Monolog\Utils;
use WPDiscourse\PhpConsole\Connector;
use WPDiscourse\PhpConsole\Handler;
use WPDiscourse\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \WPDiscourse\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \WPDiscourse\PhpConsole\Connector $connector = null, $level = \WPDiscourse\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('WPDiscourse\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\WPDiscourse\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \WPDiscourse\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \WPDiscourse\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\WPDiscourse\PhpConsole\Helper::isRegistered()) {
            \WPDiscourse\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \WPDiscourse\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \WPDiscourse\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \WPDiscourse\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \WPDiscourse\Monolog\Formatter\LineFormatter('%message%');
    }
}
