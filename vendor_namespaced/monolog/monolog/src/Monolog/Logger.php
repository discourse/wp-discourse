...
use WPDiscourse\Monolog\Handler\HandlerInterface;
use WPDiscourse\Monolog\Handler\StreamHandler;
use WPDiscourse\Psr\Log\LoggerInterface;
use WPDiscourse\Psr\Log\InvalidArgumentException;
use Exception;
/**
 * Monolog log channel
 *
 ...
 */
class Logger implements \WPDiscourse\Psr\Log\LoggerInterface, \WPDiscourse\Monolog\ResettableInterface
{
    /**
     ...
     * @return $this
     */
    public function pushHandler(\WPDiscourse\Monolog\Handler\HandlerInterface $handler)
    {
    ...
    {
        if (!$this->handlers) {
            $this->pushHandler(new \WPDiscourse\Monolog\Handler\StreamHandler('php://stderr', static::DEBUG));
        }
    ...
    public static function getLevelName($level)
    {
        if (!isset(static::$levels[$level])) {
            throw new \WPDiscourse\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
